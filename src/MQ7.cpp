...
float MQ7::readRs(){
	...
	return (float) MQ7_RL_VALUE * ((5 - VRL) / VRL);
}
/////////////////////////////////////////////////////
// Output:  Rs/Ro Ratio
float	MQ7::readRsRo (){
...
void MQ7::refresh(){
	...
	switch (m_Status){
		...
		case MQ_CALIBRATION:					// now calculate Ro
			sample = readSample();
			if(sample!=-1){
				m_Ro		 = sample / MQ7_CLEAN_AIR_FACTOR;
				m_Status	 = MQ_READY;
			}
			break;
...
int*	MQ7::getVals	(){
	...
	retVal[0] = read();
	retVal[1] = readPPM(GAS_CARBON_MONOXIDE);
	retVal[2] = readPPM(GAS_HYDROGEN);
	retVal[3] = readPPM(GAS_LPG);
...
{ 
	...
	if ( rs_ro_ratio != -1 ){
		if ( gas_id == GAS_CARBON_MONOXIDE ) 
			PPM = pow(10,(1.595 - 1.928 * log10(rs_ro_ratio)));
		if ( gas_id == GAS_HYDROGEN ) 
			PPM = pow(10,(1.333 - 1.775 * log10(rs_ro_ratio)));
		if ( gas_id == GAS_LPG ) 
			PPM = pow(10,(10.892 - 10.469 * log10(rs_ro_ratio)));
		if ( gas_id == GAS_METHANE ) 
			PPM = pow(10,(14.03 - 10.929 * log10(rs_ro_ratio)));
		if ( gas_id == GAS_ALCOHOL ) 
			PPM = pow(10,(24.181 - 18.645 * log10(rs_ro_ratio)));
...
char*	MQ7::print(){
	...
	if(m_Status == MQ_NOTCONNECTED)
		sprintf(m_buffer, "MQ7: Not Connected!");	
	if(m_Status == MQ_PREHEAT)
		sprintf(m_buffer, "MQ7: Preheat to get stable readings (%d)....", read ());
	if(m_Status == MQ_CALIBRATION)
		sprintf(m_buffer, "MQ7: Calebration (Reading Ro)....");
	if(m_Status == MQ_READY){
		vals = getVals();
		sprintf(m_buffer, "MQ7: Native= %d, CO= %dppm, H= %dppm, LPG= %dppm, METHANE= %dppm, ALCOHOL= %dppm",vals[0],vals[1],vals[2],vals[3],vals[4],vals[5]);
	}
